...
async fn test_temp_mongo_docker() {
	...
	let mongo_client = temp_mongo_docker.mongo_client.as_ref().expect("MongoDB client not initialized");
	let database = mongo_client.database("test");
	let collection = database.collection::<Document>("foo");

	// Insert a document
	let_assert!(Ok(id) = collection.insert_one(doc! { "hello": "docker world" }, None).await);
	let_assert!(Some(id) = id.inserted_id.as_object_id());
